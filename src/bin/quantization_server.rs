// Zeta Reticula's Fandango Quantization Server
// 
...
impl QuantizedModel {
    fn quantize(weights: &Tensor, bits: usize) -> anyhow::Result<Self> {
    ...
    fn dequantize(&self) -> anyhow::Result<Tensor> {
        let dequantized = (self.weights.to_dtype(DType::F32)? * self.scale)? - self.zero_point;
        Ok(dequantized)
    }
    
    fn quantized_matmul(&self, x: &Tensor) -> anyhow::Result<Tensor> {
